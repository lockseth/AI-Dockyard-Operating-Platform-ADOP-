...
// response shape — { event: { id, message } } or { event: null } — matches
// /api/internal/notifications/claim's own response shape on purpose, so the
// n8n workflow's "Has Event?" branch and the Fonnte send + complete/fail
// steps that follow are the exact same graph shape as the existing Gate 1L
// workflow; completion/failure of the send is reported back via the
// existing, unmodified /api/internal/notifications/{complete,fail} routes.
