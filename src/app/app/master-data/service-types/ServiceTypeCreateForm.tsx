...
import { createServiceTypeAction } from "@/lib/master-data/service-types/actions";
...
export function ServiceTypeCreateForm() {
  const [state, formAction, isPending] = useActionState(createServiceTypeAction, initialState);
  ...
  return (
    ...
    >
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <TextField label="Urutan" name="sortOrder" type="number" defaultValue={0} errors={state.fieldErrors?.sortOrder} />
      </div>
      ...
      <div>
        ...
        >
          {isPending ? "Menyimpan..." : "Simpan Service Type"}
        </button>
      </div>
    </form>
  );
