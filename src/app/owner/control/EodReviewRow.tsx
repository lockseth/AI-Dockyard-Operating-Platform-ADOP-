    ...
    <li className="rounded-lg border border-neutral-200 p-4 dark:border-neutral-800">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <span className="font-medium">
          {reconciliation.business_date ? formatBusinessDateLabel(reconciliation.business_date) : "-"}
        </span>
        <span className="text-xs font-medium uppercase tracking-wide text-neutral-500">
          {getCashReconciliationStatusLabel(reconciliation.status)}
          {!isLatestForPool ? " · Riwayat" : ""}
        </span>
      </div>
      ...
      {canDecide && unresolvedExpenseCount > 0 ? (
        <p role="alert" className="mt-2 text-sm text-amber-600 dark:text-amber-400">
