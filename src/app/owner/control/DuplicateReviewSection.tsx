...
export function DuplicateReviewSection({ items }: { items: DuplicateReviewItem[] }) {
  return (
    <section id="tinjauan-duplikasi" className="rounded-lg border border-neutral-200 p-6 dark:border-neutral-800">
      <h2 className="text-lg font-semibold tracking-tight">3. Tinjauan Kandidat Duplikasi</h2>
      <p className="mt-1 text-xs text-neutral-500">
        Kandidat duplikasi hanya indikasi, bukan bukti kecurangan — setiap kandidat membutuhkan keputusan Anda.
      </p>

      {items.length === 0 ? (
        <p className="mt-3 text-sm text-neutral-500">Tidak ada kandidat duplikasi yang menunggu keputusan.</p>
      ) : (
