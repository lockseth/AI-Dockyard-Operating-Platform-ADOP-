...
- **Sebelum implementasi**, ditanya via `AskUserQuestion` soal cakupan — karena ADOP punya sistem warna semantik (hijau=sukses, kuning=warning, **merah=bahaya/kandidat duplikat**) dan "Sunny day" tidak punya varian merah, risiko mengaburkan sinyal bahaya kalau diterapkan serampangan ke semua panel. User pilih: **cuma panel netral/informasi**, panel status (sukses/warning/bahaya) tetap seperti sekarang.
- Investigasi kode: hanya ada 2 tempat panel netral/info benar-benar dipakai di seluruh app — kartu hero "Posisi Kas" (`.adop-panel-primary` + `<Card tone="info">`) dan tile indikator "Proyek Kapal Aktif" (`.adop-panel-info`), keduanya di [OwnerSummarySection.tsx](src/app/owner/control/OwnerSummarySection.tsx). Panel success/warning/danger (`.adop-panel-success/-warning/-danger`) **tidak disentuh sama sekali**.
- Perubahan:
  - [Card.tsx](src/components/ui/Card.tsx) — tone `info` (`border-adop-accent-200 bg-adop-accent-50`) diganti tint sky-blue dari `#007EFF` (`border-[#bfe2ff] bg-[#eaf6ff]`), dark mode tidak diubah (sudah biru).
  - [globals.css](src/app/globals.css) — `.adop-panel-info` jadi gradient pale sky-blue → putih (`color-mix(in srgb, #007eff 12%, white)`); `.adop-panel-primary` jadi gradient dua-hue emas→biru (`color-mix(#ffbf00 18%..)` → `color-mix(#007eff 14%..)`), evokes "matahari→langit" khusus di kartu hero ini saja.
  - **Keputusan desain sengaja**: emas (`#FFBF00`) TIDAK dipakai sebagai warna dominan/border panel info, karena ADOP sudah punya `--color-brand-gold` yang dikunci khusus untuk "Owner Action surface" (`Card.tsx` tone `owner`) — dipakai hanya sebagai aksen gradasi di satu kartu hero supaya tidak bentrok makna dengan lock yang sudah ada.
- Verifikasi: dev server + ephemeral owner-role test user (Tenant A) untuk login ke `/owner/control`. Computed style dicek via JS: `.adop-panel-primary` → `linear-gradient(135deg, rgb(255,243,209) 0%, rgb(219,237,255) 100%)`; `.adop-panel-info` → `linear-gradient(135deg, rgb(224,240,255) 0%, #fff 100%)` — sesuai target, `color-mix()` resolve dengan benar di browser. Tidak ada error console. `npx tsc --noEmit` bersih (di luar 2 error pre-existing file uncommitted lain). User ephemeral gagal dihapus (kena guard trigger last-owner, pola yang sudah pernah terjadi sebelumnya — aman ditinggal, data Docker lokal saja); skrip throwaway sudah dihapus.
